...
//In Unity, the surf function is used in a similar way to access the surface properties of a mesh when writing shaders.
//It is typically used to calculate the color of a pixel or fragment in a rendering pipeline.
// It is important to note that the surf function is specific to the HLSL programming language and is not available in

// other shader languages like GLSL or Cg.
// In Unity, the surf function is typically used in conjunction with the SurfaceOutputStandard structure,
// which defines the surface properties that are available to a shader.
